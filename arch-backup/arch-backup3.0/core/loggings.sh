...
# v1.2 (2025-04-01):
#   - 增强日志缓冲区刷新机制 (错误时重试创建目录, ERROR级别以上立即刷新, 关键操作前刷新)。
#   - 改进日志文件轮转功能 (添加文件操作错误处理, 增强文件大小检查健壮性)。
#   - 优化进度条显示 (减小宽度至30, 使用 '=' 和 ' ' 符号)。
#   - 添加更多日志上下文信息 (任务ID, 进度日志记录)。
#   - 增强文件操作的错误处理 (mkdir, echo, rm, mv, stat)。
#############################################################
...
# : "${LOG_FILE:=/var/logs/script.log}"       # 默认日志文件路径（当 LOG_TO_FILE=true 时生效）。可被环境变量 LOG_FILE 覆盖。
...
init_logging() {
    ...
    if [[ "$LOG_TO_FILE" == "true" ]]; then
        ...
        if ! mkdir -p "$log_dir"; then
            LOG_FILE_PATH="" # 清空路径以禁用文件日志 先赋值是为了避免执行log_error错误，log_messege会检测这个值是否为空
            log_section "无法创建日志目录: $log_dir，已禁用文件日志记录。" $LOG_LEVEL_ERROR
        else
            log_section "初始化*创建日志目录: $log_dir" $LOG_LEVEL_NOTICE
        fi

        # 仅当日志文件路径有效（目录创建成功或已存在）时，才写入日志头信息。
        if [[ -n "$LOG_FILE_PATH" ]]; then
            ...
            {
                echo "===== 日志开始于 $(date) ====="
                echo "===== 任务 ID: $TASK_ID =====" # 新增：记录任务ID
                echo "===== 日志级别: $(get_log_level_name $CURRENT_LOG_LEVEL) ====="
                ...
                log_error "无法写入日志文件头: $LOG_FILE_PATH，已禁用文件日志记录。"
                LOG_FILE_PATH="" # 写入失败，禁用文件日志
            }
        fi
    else
    ...
    log_debug "日志系统初始化完成 (File logging: $LOG_TO_FILE, Path: ${LOG_FILE_PATH:-'N/A'}, Task ID: $TASK_ID)"
}

# 函数：check_log_file_rotation
# 功能：检查当前日志文件大小是否超过限制，如果超过则执行轮转。
# 轮转逻辑：
#   0. 主动刷新缓冲区，确保轮转前日志已写入。
...
check_log_file_rotation() {
    ...
    # 增强：添加更健壮的错误处理。
    local file_size stat_rc
    file_size=$(stat -c %s "$LOG_FILE_PATH" 2>/dev/null)
    stat_rc=$?
    # 如果 Linux stat 失败，尝试 macOS/BSD stat
    if [[ $stat_rc -ne 0 ]]; then
        file_size=$(stat -f %z "$LOG_FILE_PATH" 2>/dev/null)
        stat_rc=$?
    fi
    # 如果两种 stat 都失败，记录警告并返回
    if [[ $stat_rc -ne 0 ]]; then
        log_warn "无法获取日志文件大小: $LOG_FILE_PATH"
        return
    fi

    # 如果获取大小失败（理论上不会到这里）或文件大小未超过限制，则返回。
    ...
    if [[ -f "$oldest_log" ]]; then
        rm -f "$oldest_log" || log_error "无法删除旧日志文件: $oldest_log"
    fi
    ...
    for ((i = LOG_FILE_MAX_COUNT - 1; i >= 1; i--)); do
        ...
        if [[ -f "$current_log" ]]; then
            mv "$current_log" "$next_log" || log_error "无法轮转日志文件: $current_log -> $next_log"
        fi
    done

    # 3. 将当前日志文件移动为 .1
    if [[ -f "$LOG_FILE_PATH" ]]; then # 再次检查，以防万一
        mv "$LOG_FILE_PATH" "${LOG_FILE_PATH}.1" || log_error "无法重命名当前日志文件: $LOG_FILE_PATH -> ${LOG_FILE_PATH}.1"
    fi
    ...
    # 注意：这里不应再次调用 init_logging，只需创建新文件和写入必要的头信息。
    {
        ...
        echo "===== 日志级别: $(get_log_level_name $CURRENT_LOG_LEVEL) ====="
        echo ""
    } > "$LOG_FILE_PATH" || log_error "无法创建新的轮转后日志文件: $LOG_FILE_PATH"
...
#       在缓冲区满、脚本退出、ERROR级别以上日志或需要确保日志立即写入时调用。
# 增强：添加错误处理，尝试重新创建目录。
...
flush_log_buffer() {
    ...
    if [[ -n "$LOG_BUFFER" && -n "$LOG_FILE_PATH" ]]; then
        ...
        # 追加 (>>) 到日志文件。
        if ! printf "%b" "$LOG_BUFFER" >> "$LOG_FILE_PATH"; then
            local write_error=$? # 保存错误码
            local log_dir
            log_dir=$(dirname "$LOG_FILE_PATH")
            # 记录第一次写入失败
            log_error "写入日志文件失败 (错误码: $write_error): $LOG_FILE_PATH。尝试重新创建目录..."

            # 尝试重新创建目录
            if ! mkdir -p "$log_dir"; then
                log_error "重新创建日志目录失败: $log_dir。日志可能丢失。"
                # 清空缓冲区，避免无限重试
                LOG_BUFFER=""
                LOG_BUFFER_SIZE=0
                return 1 # 返回错误
            ...
            if ! printf "%b" "$LOG_BUFFER" >> "$LOG_FILE_PATH"; then
                 local retry_error=$?
                 log_error "重试写入日志文件仍然失败 (错误码: $retry_error): $LOG_FILE_PATH。日志可能丢失。"
                 # 清空缓冲区
                 LOG_BUFFER=""
                 LOG_BUFFER_SIZE=0
                 return 1 # 返回错误
            else
                 log_warn "成功重新创建目录并写入日志: $LOG_FILE_PATH"
                 # 写入成功，清空缓冲区
                 LOG_BUFFER=""
                 LOG_BUFFER_SIZE=0
            fi
...
add_to_log_buffer() {
    ...
    if [[ -n "$LOG_FILE_PATH" ]]; then
        ...
        if [[ $LOG_BUFFER_SIZE -ge $LOG_BUFFER_MAX_SIZE ]]; then
            flush_log_buffer || log_error "缓冲区满时刷新日志失败" # 记录刷新错误
        fi
    fi
}
...
log_message() {
    ...
    # 获取通用信息
    local log_timestamp
    log_timestamp=$(date "+%Y-%m-%d %H:%M:%S") # 标准时间戳格式
    local level_name
    ...
    if [[ "$COLOR_OUTPUT" == "true" ]]; then
        echo -e "${COLOR_BOLD}[${log_timestamp}]${caller_info} ${level_color}[${level_name}]${COLOR_RESET} ${level_color}${message}${COLOR_RESET}"
    else
        echo "[${level_name}] [${log_timestamp}]${caller_info} ${message}"
    fi

    # --- 文件输出 (通过缓冲区) ---
    if [[ -n "$LOG_FILE_PATH" ]]; then
        # 构建用于文件的日志条目（无颜色代码）。
        local log_entry="[${log_timestamp}] [${level_name}]${caller_info} ${message}"
        add_to_log_buffer "$log_entry"

        # 增强：对于错误级别及以上的日志，立即刷新缓冲区
        if [[ $level -ge $LOG_LEVEL_ERROR ]]; then
            flush_log_buffer || log_error "立即刷新错误/严重日志失败" # 记录刷新错误
        fi
...
log_format() {
    ...
    local message
    # 添加错误处理，防止 printf 失败（例如格式字符串无效）
    if ! message=$(printf "$format" "$@"); then
        log_error "格式化日志消息失败。格式: '$format', 参数: '$*'"
        message="[格式化错误] $format $*" # 提供回退消息
    fi
...
log_section() {
    ...
    if [[ -n "$LOG_FILE_PATH" ]]; then
        # 文件日志中包含级别和时间戳。
        local log_entry="[${log_timestamp}] [${level_name}] ${plain_line}" # 不包含 caller_info
        add_to_log_buffer "$log_entry"
...
finalize_logging() {
    ...
    flush_log_buffer || log_error "退出时刷新日志缓冲区失败"
    ...
    if [[ -n "$LOG_FILE_PATH" ]]; then
        ...
        if [[ -f "$LOG_FILE_PATH" ]]; then
             # 添加错误处理
             {
                 ...
                 echo "===== 任务 ID: $TASK_ID =====" # 新增：记录任务ID
                 echo "===== 日志结束于 $(date) ====="
             } >> "$LOG_FILE_PATH" || log_error "无法写入日志文件尾部: $LOG_FILE_PATH"
        fi
    fi
}
...
show_progress_bar() {
    ...
    if [[ $current -le 1 ]]; then
        LAST_LOGGED_PERCENT=-1 # 重置上次记录的百分比
        echo "" # 前导空行
        if [[ -n "$description" ]]; then # 仅当有描述时打印开始信息
             if [[ "$COLOR_OUTPUT" == "true" ]]; then
                 echo -e "${COLOR_BOLD}${COLOR_BG_CYAN} 开始 ${COLOR_RESET} ${COLOR_BOLD}${COLOR_CYAN}$description${COLOR_RESET} ${COLOR_CYAN}(Task: $TASK_ID)${COLOR_RESET}"
             else
                 echo "[开始] $description (Task: $TASK_ID)"
             fi
        fi
        # 打印分隔线
        if [[ "$COLOR_OUTPUT" == "true" ]]; then
            echo -e "${COLOR_CYAN}--------------------------------------------------${COLOR_RESET}" # 稍微缩短
        else
            echo "--------------------------------------------------"
        fi
    fi
    ...
    # 新增：每10%记录一次日志
    # 使用整数除法比较，避免边界问题
    if (( percentage / 10 > LAST_LOGGED_PERCENT / 10 && percentage < 100 )); then
        log_debug "进度 ($description): ${percentage}% (Task: $TASK_ID)"
    ...
    if [[ "$COLOR_OUTPUT" == "true" ]]; then
        ...
        local i
        for ((i=0; i<completed; i++)); do progress+="="; done # 优化：使用 '='
        progress+="${COLOR_RESET}" # 重置颜色
        for ((i=0; i<remaining; i++)); do progress+=" "; done # 优化：使用 ' '
    ...
    if [[ $current -eq $total ]]; then
        # 确保记录100%的日志（如果尚未记录）
        if [[ $LAST_LOGGED_PERCENT -lt 100 ]]; then
             log_debug "进度 ($description): 100% (Task: $TASK_ID)"
        fi
        echo "" # 换行，结束进度条更新
        if [[ -n "$description" ]]; then # 仅当有描述时打印完成信息
            if [[ "$COLOR_OUTPUT" == "true" ]]; then
                echo -e "${COLOR_BOLD}${COLOR_BG_GREEN} 完成 ${COLOR_RESET} ${COLOR_BOLD}${COLOR_GREEN}$description${COLOR_RESET} ${COLOR_CYAN}($(date +"%H:%M:%S"), Task: $TASK_ID)${COLOR_RESET}"
                # 添加分隔线
                echo -e "${COLOR_CYAN}--------------------------------------------------${COLOR_RESET}"
            else
                echo "[完成] $description ($(date +"%H:%M:%S"), Task: $TASK_ID)"
                echo "--------------------------------------------------"
            fi
        fi
         echo "" # 完成后添加额外空行
...
show_progress_percent() {
    ...
    if [[ $current -le 1 ]]; then
        ...
        echo "" # 前导空行
         if [[ -n "$description" ]]; then # 仅当有描述时打印开始信息
             if [[ "$COLOR_OUTPUT" == "true" ]]; then
                 echo -e "${COLOR_BOLD}${COLOR_BG_PURPLE} 进度 ${COLOR_RESET} ${COLOR_BOLD}${COLOR_LIGHT_PURPLE}$description${COLOR_RESET} ${COLOR_CYAN}(Task: $TASK_ID)${COLOR_RESET}"
             else
                 echo "[进度] $description (Task: $TASK_ID)"
             fi
         fi
        # 打印分隔线
        if [[ "$COLOR_OUTPUT" == "true" ]]; then
            echo -e "${COLOR_PURPLE}--------------------------------------------------${COLOR_RESET}" # 稍微缩短
        else
            echo "--------------------------------------------------"
        fi
    fi
...
show_progress() {
    ...
    if [[ $total -eq 0 ]]; then
        # 可以考虑记录一个警告或调试信息
        log_warn "show_progress 调用时 total=0 (描述: $description)"
        return
    fi
...
show_progress_with_eta() {
    ...
    if [[ $total -eq 0 ]]; then
        log_warn "show_progress_with_eta 调用时 total=0 (描述: $description)"
        return
    fi
    if ! [[ "$start_time" =~ ^[0-9]+$ ]]; then
         log_error "show_progress_with_eta 需要有效的开始时间戳 (第四个参数)"
         # 仍然尝试显示进度，但不显示ETA
         show_progress "$current" "$total" "$description" "ETA错误:无效开始时间"
         return
    fi
    ...
    if command -v bc &> /dev/null; then
        if [[ $current -gt 0 ]]; then
            ...
            # 添加错误处理，以防 bc 计算出错
            rate=$(echo "scale=4; if ($current > 0) $elapsed / $current else 0" | bc 2>/dev/null)
            if [[ -n "$rate" && "$rate" != "0" ]]; then # 检查 rate 是否有效且非零
...
# --- 兼容旧版 log 函数 ---
# 功能：提供对旧版 log "LEVEL" "message" 调用的兼容性。
#       解析 LEVEL 并调用对应的新版 log_level 函数。
# 参数：
#   $1 - 日志级别字符串 (INFO, WARN, ERROR, FATAL, DEBUG)
#   $2 - 日志消息内容
log() {
    local level_str="${1^^}" # 转为大写
    local message="$2"
    ...
    case "$level_str" in
        ...
        "FATAL")
            log_fatal "$message"
            # 模拟旧版 FATAL 行为：记录日志后退出
            # 尝试获取日志文件路径，如果未初始化则使用默认值
            local current_log_file=${LOG_FILE_PATH:-${LOG_FILE:-/tmp/script.log}}
            echo -e "${COLOR_LIGHT_PURPLE}备份过程中遇到致命错误 (来自旧版 log 调用)，退出脚本${COLOR_RESET}" | tee -a "$current_log_file" >&2
            exit 1
            ;;
        *)
            # 对于未知的旧级别，默认使用 INFO 级别记录，并添加警告
            log_warn "收到来自旧版 log 调用的未知级别 '$1'，消息: $message"
            log_info "$message" # 同时用 INFO 级别记录原始消息
            ;;
    esac
}
