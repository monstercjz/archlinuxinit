...
get_manually_installed_packages() {
    ...
    local explicit_packages=$(pacman -Qe)
    local aur_packages=$(pacman -Qm)
    ...
    while read -r pkg version; do
        ...
        package_info["$pkg,version"]="$version"
    done <<< "$aur_packages"
    ...
    while read -r pkg version; do
        ...
        if [ -n "$install_log" ]; then
            local pkg_date
            # 使用bash内置的正则表达式匹配，提高效率
            if [[ "$install_log" =~ \[([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}[+-][0-9]{4})\] ]]; then
                pkg_date=$(date -d "${BASH_REMATCH[1]}" +%s 2>/dev/null)
            else
                # 使用参数扩展替代cut命令
                local datetime=${install_log%% [*}
                pkg_date=$(date -d "$datetime" +%s 2>/dev/null)
            fi
            
            # 如果日期转换成功且在系统安装日期之后
            if [ -n "$pkg_date" ] && [ "$pkg_date" -gt "$install_date_epoch" ]; then
                # 使用缓存的包信息
                if [ -z "${package_info[$pkg,size]}" ]; then
                    package_info["$pkg,size"]=$(pacman -Qi "$pkg" | grep "安装后大小" | cut -d':' -f2 | tr -d ' ')
                fi
                ...
                # 将结果添加到数组，使用缓存的大小信息
                results+=("$(date -d @"$pkg_date" "+%Y-%m-%d %H:%M:%S")|$pkg|${package_info[$pkg,size]}|$repo")
            fi
        fi
    done <<< "$explicit_packages"
    ...
    case "$sort_by" in
        "date")
            ...
            IFS=$'\n' sorted_results=($(sort -r -t'|' -k1 <<< "${sorted_results[*]}"))
            ;;
        "size")
            echo -e "${GREEN}按软件包大小排序（最大的在前）${NC}" >&2
            IFS=$'\n' sorted_results=($(sort -t'|' -k3hr <<< "${sorted_results[*]}"))
            ;;
        "name")
            echo -e "${GREEN}按软件包名称排序（字母顺序）${NC}" >&2
            IFS=$'\n' sorted_results=($(sort -t'|' -k2 <<< "${sorted_results[*]}"))
            ;;
        *)
