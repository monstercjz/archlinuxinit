...
#         3. 统计结果和软件包名称列表将保存到 save_dir="$HOME/arch-linux-init/info-logs" 目录。
#         4. 可以使用生成的软件包名称列表在新系统上一键安装所有软件包:
#            sudo pacman -S --needed - < $manual_list_file
# 依赖:   bash, pacman, journalctl, bc
# 注意:   部分功能可能需要root权限。
# 特别说明： 本脚步获取的实际为系统安装的实际，pacmaninstall.log里面的时间，在系统安装期间的时间显示的非本地时间
# 并且获取的系统安装时间，也并非系统用户手动安装的最早时间
# 通过pacman -Qe获取的软件包，然后去日志里对比安装时间，获取手动安装的软件包
# ==============================================================================
# 颜色定义
