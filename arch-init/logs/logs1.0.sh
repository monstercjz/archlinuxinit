...
# These can be overridden by exporting the variables before sourcing this script.
: "${LOG_LEVEL:=INFO}"                 # Default log level
: "${COLOR_OUTPUT:=true}"              # Enable color output by default (can be set to false)
: "${LOG_TO_FILE:=false}"              # Disable file logging by default
: "${LOG_FILE:=/tmp/script.log}"       # Default log file path if LOG_TO_FILE is true
: "${LOG_FILE_MAX_SIZE:=10485760}"     # Default max log size (10MB)
: "${LOG_FILE_MAX_COUNT:=5}"           # Default max log file count
: "${VERBOSE:=false}"                  # Disable verbose caller info by default
: "${SHOW_PROGRESS:=true}"             # Enable progress display by default
: "${PROGRESS_TYPE:=bar}"              # Default progress type ('bar' or 'percent')

# 当前日志级别 (Set based on LOG_LEVEL after defaults are applied)
CURRENT_LOG_LEVEL=$LOG_LEVEL_INFO # Initial default, will be updated in set_log_level
...
# 初始化日志系统
init_logging() {
    # 设置日志级别 (using the potentially defaulted LOG_LEVEL)
    set_log_level "$LOG_LEVEL"

    # 设置日志文件路径 (using potentially defaulted LOG_TO_FILE and LOG_FILE)
    if [[ "$LOG_TO_FILE" == "true" ]]; then
        LOG_FILE_PATH="$LOG_FILE"
        # 确保日志目录存在
        mkdir -p "$(dirname "$LOG_FILE_PATH")" || { log_error "无法创建日志目录: $(dirname "$LOG_FILE_PATH")"; LOG_FILE_PATH=""; } # Add error handling
        ...
        LOG_FILE_PATH="" # Ensure path is empty if file logging is disabled
    fi
...
log_message() {
    ...
    # 如果是调试或跟踪级别，添加调用者信息 (using potentially defaulted VERBOSE)
...
log_section() {
    ...
    plain_line+=" ## $title ## "
    for ((i=0; i<padding; i++)); do plain_line+="="; done
    # Ensure minimum width for plain line
    while [[ ${#plain_line} -lt $width ]]; do plain_line+="="; done

    # --- Build Colored Line (if enabled) ---
    if [[ "$COLOR_OUTPUT" == "true" ]]; then
        local level_color=$(get_log_level_color $level)
        colored_line+="${COLOR_BOLD}${level_color}"
        for ((i=0; i<padding; i++)); do colored_line+="━"; done
        colored_line+=" ★ $title ★ "
        for ((i=0; i<padding; i++)); do colored_line+="━"; done
        # Ensure minimum width for colored line (approximate)
    ...
    if [[ $CURRENT_LOG_LEVEL -le $level ]]; then
        # Console Output
        echo "" # Add preceding newline for readability
        if [[ "$COLOR_OUTPUT" == "true" ]]; then
            echo -e "$colored_line"
        else
            echo "$plain_line"
        fi
        echo "" # Add trailing newline for readability

        # File Output (always plain text, bypass log_message console output)
        if [[ -n "$LOG_FILE_PATH" ]]; then
            local log_entry="[${level_name}] [${timestamp}] $plain_line" # No caller info for sections
            add_to_log_buffer "$log_entry"
            check_log_file_rotation # Check rotation after adding to buffer potentially triggers flush
        fi
    fi
}
...
show_progress() {
    ...
    # 如果禁用进度显示，则直接返回 (using potentially defaulted SHOW_PROGRESS)
    ...
    # 根据配置选择进度显示类型 (using potentially defaulted PROGRESS_TYPE)
...
show_progress_with_eta() {
    ...
    local eta_formatted="N/A" # Default ETA
    ...
    if command -v bc &> /dev/null; then
        ...
        if [[ $current -gt 0 ]]; then
            # 使用 bc 计算，添加错误处理以防万一
            local rate=$(echo "scale=2; $elapsed / $current" | bc 2>/dev/null)
            local remaining=$(echo "scale=2; $rate * ($total - $current)" | bc 2>/dev/null)
            
            if [[ -n "$remaining" ]]; then
                remaining=${remaining%.*} # 取整
                if [[ $remaining -gt 0 ]]; then
                    eta_formatted=$(format_time "$remaining")
                else
                    eta_formatted="即将完成"
                fi
            else
                 eta_formatted="计算错误" # Handle potential bc errors
            fi
        else
             eta_formatted="未知" # Cannot calculate rate if current is 0
        fi
    else
        eta_formatted="N/A (bc missing)" # Indicate bc is missing
    fi
