...
verify_backup() {
    ...
    if [ "$COMPRESS_BACKUP" == "true" ]; then
        ...
        if [ "$BACKUP_SYSTEM_CONFIG" == "true" ] && [ ! -d "${BACKUP_DIR}/etc" ]; then
            log "ERROR" "验证失败: 系统配置目录不存在"
            verify_status=1
        fi
        
        if [ "$BACKUP_USER_CONFIG" == "true" ] && [ ! -d "${BACKUP_DIR}/home" ]; then
            log "ERROR" "验证失败: 用户配置目录不存在"
            verify_status=1
        fi
        
        if [ "$BACKUP_PACKAGES" == "true" ] && [ ! -d "${BACKUP_DIR}/packages" ]; then
            log "ERROR" "验证失败: 软件包目录不存在"
            verify_status=1
        fi
        
        if [ "$BACKUP_LOGS" == "true" ] && [ ! -d "${BACKUP_DIR}/logs" ]; then
            log "ERROR" "验证失败: 日志目录不存在"
            verify_status=1
        fi
