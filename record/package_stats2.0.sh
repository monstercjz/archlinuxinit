...
get_manually_installed_packages() {
    ...
    for pkg in $explicit_packages; do
        ...
        if [ -n "$install_log" ]; then
            ...
            # 如果是中文格式的日期，转换为date命令可以理解的格式
            if [[ "$pkg_date" =~ ([0-9]{4})年([0-9]{2})月([0-9]{2})日[[:space:]]星期[一二三四五六日][[:space:]]([0-9]{2})时([0-9]{2})分([0-9]{2})秒 ]]; then
                # 提取日期部分和时间部分
                year="${BASH_REMATCH[1]}"
                ...
                pkg_date_for_compare="$year-$month-$day $hour:$minute:$second"
            fi
