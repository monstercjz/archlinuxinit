...
main() {
    ...
    #     if command -v generate_checksum_manifest &>/dev/null; then
    #         generate_checksum_manifest "$BACKUP_DIR" || log "WARN" "生成校验和清单失败"
    #     else
    #         log "WARN" "generate_checksum_manifest 函数未找到，无法生成校验和清单。"
    #     fi
    # else
    #     log "INFO" "跳过为未压缩文件生成校验和清单，因为备份将被压缩。"
    # fi
    ...
    # 创建备份摘要
    # 计算总耗时
    local main_end_time main_duration
    main_end_time=$(date +%s)
    main_duration=$((main_end_time - main_start_time))
    # 将耗时和错误数传递给摘要函数
    create_backup_summary "$main_duration" "$backup_errors" || log "WARN" "创建备份摘要失败"
    ...
    if [ "$VERIFY_BACKUP" == "true" ]; then
        # 确保 verify_backup 函数存在 (它应该指向 verify_system)
        if command -v verify_system &>/dev/null; then
            verify_system || log "WARN" "验证备份失败，备份可能不完整"
        else
             log "ERROR" "verify_system 函数未找到，无法执行备份验证！"
             # 可以选择将此视为错误并增加 backup_errors 计数
             # backup_errors=$((backup_errors + 1))
        fi
    fi
